...
import { RO1_DEFAULT_URL, loadRo1Url } from '@/lib/appConfig';
...
export default function Ro1Screen() {
    const router = useRouter();
    const webViewRef = useRef<WebView>(null);
    const [canGoBack, setCanGoBack] = useState(false);
    const [loading, setLoading] = useState(true);
    const [ro1Url, setRo1Url] = useState(RO1_DEFAULT_URL);
    ...
    useEffect(() => {
        ...
        (async () => {
            const url = await loadRo1Url(RO1_DEFAULT_URL);
            if (!cancelled) {
                setRo1Url(url);
            }
        })();
